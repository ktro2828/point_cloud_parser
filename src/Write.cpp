...
namespace point_cloud_io {
    Write::Write(ros::NodeHandle &nodeHandle) : nodeHandle_(nodeHandle), filePrefix_("point_cloud"), fileEnding("ply") {
        ...
        pointCloudSubscriber_ = this->nodeHandle_.subscribe(this->pointCloudTopic_, 1, &Write::pointCloudCallback, this);
        ROS_INFO_STREAM("Subscribed to topic \"" << this->pointCloudTopic_ << "\".");
    }
    ...
    void Write::pointCloudCallback(const sensor_msgs::PointCloud2ConstPtr &cloud) {
        ROS_INFO_STREAM("Recieved point cloud with " << cloud->height * cloud->width << " points.");
        ...
        if (fileEnding_ == "ply") {
            ...
            ROS_ERROR_STREAM("Data format " << fileEnding_ << " is unsupported.");
            return;
        }
